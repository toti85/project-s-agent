...
class AIClient:
    ...
    def list_available_models(self) -> List[Dict[str, Any]]:
        ...
        # Skip non-provider keys like 'task_model_mapping' and 'default_model'
        skip_keys = {"task_model_mapping", "default_model"}
        
        # Go through all providers and their models
        for provider, provider_config in self.config.items():
            if provider in skip_keys:
                continue
                    ...
                    for model_id, model_config in models_dict.items():
                        if isinstance(model_config, dict):
                            available_models.append({
                                ...
                                "max_tokens": model_config.get("context_length", model_config.get("max_tokens", 4096)),
    ...
    def _get_actual_model_id(self, config_model_id: str, provider: str) -> str:
        """
        Get the actual API model ID from configuration.
        For OpenRouter, uses the model_id field if available, otherwise uses the config key.
        """
        provider_config = self.config.get(provider, {})
        model_config = provider_config.get("models", {}).get(config_model_id, {})
        
        # For OpenRouter, use the model_id field if available
        if provider == "openrouter" and isinstance(model_config, dict):
            return model_config.get("model_id", config_model_id)
        
        # For other providers, use the config key
    ...
    def generate_response(
        ...
        try:
            ...
            if selected_provider == "openai":
                return self._call_openai(selected_model, prompt, system_message, temperature, max_tokens)
            elif selected_provider == "anthropic":
                return self._call_anthropic(selected_model, prompt, system_message, temperature, max_tokens)
            elif selected_provider == "openrouter":
                # For OpenRouter, use the actual API model ID
                actual_model_id = self._get_actual_model_id(selected_model, selected_provider)
                return self._call_openrouter(actual_model_id, prompt, system_message, temperature, max_tokens)
            elif selected_provider == "ollama":
                return self._call_ollama(selected_model, prompt, system_message, temperature, max_tokens)
            else:
    ...
    def _call_openrouter(self, model: str, prompt: str, system_message: Optional[str] = None,
        ...
        try:
            ...
            # OpenRouter specific headers (optional according to documentation)
            extra_headers = {
