    ...
    async def _create_workflow_graph(self):
        ...
        async def fetch_web_content(state: Dict[str, Any]) -> Dict[str, Any]:
            ...
            try:
                ...
                logger.info(f"✅ Weboldal tartalom sikeresen letöltve (hossz: {len(content)} karakter)")
                logger.debug(f"[DEBUG] fetch_web_content - content preview: {content[:200]}...")
            except Exception as e:
        ...
        # 3. Tartalom elemzése        async def analyze_content(state: Dict[str, Any]) -> Dict[str, Any]:
            logger.info("🔍 Tartalom elemzése (DEBUG)")
            logger.debug(f"[DEBUG] analyze_content state keys: {list(state.keys())}")
            logger.debug(f"[DEBUG] content length: {len(state.get('content', ''))}")
            new_state = state.copy()
            try:
        ...
        async def general_summary(state: Dict[str, Any]) -> Dict[str, Any]:
            ...
            try:
                ...
                # Állapot frissítése                new_state["summary_path"] = summary_path
        ...
        async def generate_recommendations(state: Dict[str, Any]) -> Dict[str, Any]:
            ...
            try:
                ...
                # Állapot frissítése                new_state["recommendations_path"] = recommendations_path
        ...
        builder.add_edge("initialize", "fetch_content")        # Alapvető munkafolyamat útvonal
        ...
        builder.add_conditional_edges(
            "decide_branch",
            lambda state: state.get("branch", "general_branch"),
            {
        ...
        # Javaslatok generálása után befejezés
        builder.add_edge("generate_recommendations", "finalize")
