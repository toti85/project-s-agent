...
class PathSerializationMixin:
    """Mixin to handle Path object serialization in Pydantic models"""
    
    def dict(self, **kwargs):
        """Override dict method to handle Path objects"""
        result = super().dict(**kwargs)
        return self._serialize_paths(result)
    
    def json(self, **kwargs):
        """Override json method to handle Path objects"""
    ...
    def _serialize_paths(obj):
        ...
        if isinstance(obj, (Path, WindowsPath, PosixPath)):
            ...
            return {k: PathSerializationMixin._serialize_paths(v) for k, v in obj.items()}
        elif isinstance(obj, list):
            return [PathSerializationMixin._serialize_paths(item) for item in obj]
        return obj

# Define structured state classes for cognitive workflows
class Memory(BaseModel, PathSerializationMixin):
    conversation: List[Dict[str, Any]] = Field(default_factory=list)
    entities: Dict[str, List[str]] = Field(default_factory=dict)
    context: Dict[str, Any] = Field(default_factory=dict)

class Task(BaseModel, PathSerializationMixin):
    id: str
    description: str
...
class CognitiveGraphState(BaseModel, PathSerializationMixin):
    ...
    ai_model_config: Dict[str, Any] = Field(default_factory=dict)  # Renamed to avoid Pydantic conflict
...
class CognitiveCoreGraph:
    ...
    def _build_graph(self):
        ...
        graph.add_edge("error_handler", "reflector")        # Set the entry point
        graph.set_entry_point("planner")
          # Compile the graph with proper checkpoint configuration
        try:
            # For LangGraph 0.0.69, compile without checkpointer to avoid API compatibility issues
            compiled_graph = graph.compile()
            logger.info("LangGraph compiled successfully without checkpointer (compatible with 0.0.69)")
            return compiled_graph
        except Exception as e:
            logger.error(f"LangGraph compilation failed: {str(e)}")
            # Ultimate fallback: return None and use manual processing
            logger.warning("Using manual processing fallback due to LangGraph compilation failure")
            return None
...
class CognitiveCoreWithLangGraph:
    ...
    async def process_task(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            initial_state = CognitiveGraphState(
                ...
                ai_model_config=self.core_graph.model_config,
                status="planning"            )
            ...
            if self.core_graph.graph is None:
                logger.warning("LangGraph compilation failed, using manual processing")
                # Use manual processing
                fallback_state = CognitiveGraphState(
                    memory=initial_state.memory,
                    tasks=initial_state.tasks,
                    current_task_id=task_id,
                    active_tasks=[task_id],
                    workspace=initial_state.workspace,
                    ai_model_config=initial_state.ai_model_config,
                    status="manual_processing"
                )
                
                # Process through nodes manually
                planner_node = self.core_graph.nodes["planner"]
                executor_node = self.core_graph.nodes["executor"]
                analyser_node = self.core_graph.nodes["analyser"]
                
                # Execute the pipeline manually
                fallback_state = await planner_node(fallback_state)
                if fallback_state.status != "error":
                    fallback_state = await executor_node(fallback_state)
                if fallback_state.status != "error":
                    fallback_state = await analyser_node(fallback_state)
                
                final_state = fallback_state
                logger.info(f"Manual processing completed for task {task_id}")
            else:
                # Set up the workflow to use the core graph with proper error handling
                try:
                    logger.info(f"Invoking LangGraph for task {task_id}")
                    # Use thread_id for checkpoint tracking
                    config = {"configurable": {"thread_id": f"task_{task_id}"}}
                    final_state = await self.core_graph.graph.ainvoke(initial_state, config=config)
                    logger.info(f"LangGraph execution completed for task {task_id}")
                except Exception as langgraph_error:
                    logger.error(f"LangGraph execution failed: {str(langgraph_error)}")
                    # Fallback to direct processing if LangGraph fails
                    try:
                        # Create a simple state for fallback processing
                        fallback_state = CognitiveGraphState(
                            memory=initial_state.memory,
                            tasks=initial_state.tasks,
                            current_task_id=task_id,
                        ...
                        # Process through nodes manually as fallback
                        planner_node = self.core_graph.nodes["planner"]
                        executor_node = self.core_graph.nodes["executor"]
                        analyser_node = self.core_graph.nodes["analyser"]
                        ...
                        final_state = fallback_state
                        logger.info(f"Fallback processing completed for task {task_id}")
                        
                    except Exception as fallback_error:
                        logger.error(f"Both LangGraph and fallback processing failed: {str(fallback_error)}")
                        # Create minimal error state
                        final_state = CognitiveGraphState(
                            memory=initial_state.memory,
                            tasks={task_id: Task(id=task_id, description=task.get("description", str(task)), status="failed", error={"message": str(langgraph_error)})},
                            current_task_id=task_id,
    ...
    async def process_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Wrapper around process_task for API compatibility.
        
        Args:
            request (Dict[str, Any]): The request containing query and optional parameters
            
        Returns:
            Dict[str, Any]: The result of processing
        """
        try:
            # Convert request to task format
            task = {
                "id": request.get("conversation_id", f"request_{len(self.context.get('tasks', [])) + 1}"),
            ...
            # Use existing process_task method
