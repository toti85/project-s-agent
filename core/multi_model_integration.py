...
class MultiModelManager:
    ...
    async def execute_task(self, task_type: str, prompt: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            # Check if model has ask method, if not use generate method
            if hasattr(model, 'ask'):
                response = await model.ask(prompt, **kwargs)
            elif hasattr(model, 'generate'):
                # Fallback to generate method for OllamaClient
                response = await model.generate(prompt, **kwargs)
            else:
                raise AttributeError(f"Model {model.__class__.__name__} has neither 'ask' nor 'generate' method")
                
            return {
                "status": "success",
                "model_used": getattr(model, 'model_name', model.__class__.__name__),
                "response": response
            }
        except Exception as e:
